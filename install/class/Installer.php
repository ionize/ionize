...
class Installer
{
	...
	function migrate()
	{
		...
		else
		{
			...
			/*
			 * Migration to new 0.9.9 userstable
			 * Remove salt, add forgotten password fields ...
			 */
			if (in_array('migration_0.9.9_userstable.xml', $migration_files))
			{
				log_message('debug', 'Migrate usertables of 0.9.9');
	...
	function _get_migration_files()
	{
		...
		if (($test = $this->db->query('select count(1) from setting')) != false)
		{
			...
			/*
			 * Migrate User table - only on version 99!
			 *
			...
			if( version_compare($version, "0.9.9") <= 0 ) {
				...
				if ($migrate_from == true)
					$migration_xml[] = 'migration_0.9.9_userstable.xml';
			}
