...
<?php if( !empty($extends)) :?>
	...
	<?php foreach($extends as $extend) :?>
		...
		<li class="sortme extend_field<?= $extend->name ?>" id="extend_field_<?= $extend->name ?>" rel="<?= $extend->name ?>">
			...
			<a class="left ml5" href="javascript:void(0);" onclick="javascript:ION.formWindow('extendtable', 'extendtableForm', '<?= lang('ionize_title_extend_field') ?>', 'extend_table/edit/<?=$table?>/<?= $extend->name ?>', {width: 400, height: 330, title:Lang.get('ionize_title_extend_table_field')});" title="<?= $extend->name ?>"><?= $extend->name ?> | <?= $extend->type ?></a>
		</li>
