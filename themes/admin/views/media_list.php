...
<?php if ( !empty($items)) :?>
	...
	<?php foreach ($items as $media) :?>
		...
		<li class="sortme" id="<?= $media['type'] ?>_<?= $media['id_media'] ?>">
			...
			<a class="left ml10" href="javascript:ION.formWindow('<?= $media['type'].$media['id_media'] ?>', 'mediaForm<?= $media['id_media'] ?>', '<?= $media['file_name'] ?>', 'media/edit/<?= $media['type'] ?>/<?= $media['id_media'] ?>', {width:500,height:460});" title="edit"><?= $media['file_name'] ?></a>
		</li>
