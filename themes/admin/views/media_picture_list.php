...
//	$path = str_replace($file_path, '', $media['path']);
	...
	<div class="picture drag" id="picture_<?= $media['id_media'] ?>">
		...
		<p class="icons">
			...
			<a class="icon edit left mr5 " href="javascript:MUI.formWindow('<?= $media['type'].$media['id_media'] ?>', 'mediaForm<?= $media['id_media'] ?>', '<?= $media['file_name'] ?>', '<?= admin_url(TRUE) ?>media/edit/picture/<?= $media['id_media'] ?>', {width:500,height:430,resize:true});" title="<?= lang('ionize_label_edit') ?>"></a>
